...
def run(fileList):
    ...
    image_name = Label(root,text="Image Name",borderwidth=2, relief="solid",bg = '#3ed8ea',fg='#1c1c6c',height=2,width=20,font='Verdana 12')
    ...
    hspace(5,root)
    cr = Label(root,text="Recreated Image Size",borderwidth=2, relief="solid",bg = '#3ed8ea',fg='#1c1c6c',height=2,width=25,font='Verdana 12')
    cr.grid(row=2,column=6)
    hspace(7,root)
    cr = Label(root,text="Compression Ratio",borderwidth=2, relief="solid",bg = '#3ed8ea',fg='#1c1c6c',height=2,width=20,font='Verdana 12')
    cr.grid(row=2,column=8)
    ...
    for i in range(len(fileList)):
        ...
        recreated_file_name =  "./Recreated-Images/"+name[:len(name)-4]+"_recreated"+name[len(name)-4:]
        compressed_file_name =  "./Compressed-Images/"+name[:len(name)-4]+"_compressed"+name[len(name)-4:]
        create_table(root,name,fileList[i],recreated_file_name, compressed_file_name, rows)
        rows = rows+1
    mainloop()
...
def create_table(root,name,file,file2,file3,row):
    '''
    ...
    image1 = Label(root,text = meta_data(file),bg = '#3ed8ea',fg='#1c1c6c',font='Verdana 12 underline')
    image1.grid(row=row,column = 2)
    image1.bind("<Button-1>",lambda event:leftClick(event,file))
    image1 = Label(root,text = meta_data(file3),bg = '#3ed8ea',fg='#1c1c6c',font='Verdana 12 underline')
    image1.grid(row=row,column = 4)
    image1.bind("<Button-1>",lambda event:leftClick(event,file3))
    image1 = Label(root,text = meta_data(file2),bg = '#3ed8ea',fg='#1c1c6c',font='Verdana 12 underline')
    image1.grid(row=row,column = 6)
    image1.bind("<Button-1>",lambda event:leftClick(event,file2))
    image1 = Label(root,text = extract_ratio(meta_data(file),meta_data(file3)),bg = '#3ed8ea',fg='#1c1c6c',font='Verdana 12')
    image1.grid(row=row,column = 8)

def meta_data(file):
    '''
    Gives the file size of the selected file in kB
    '''
    return str(math.ceil(int(os.path.getsize(file))/1024)) + " kB"
